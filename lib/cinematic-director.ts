...
export const NEW_YORK_BIRDS_EXIT = 28.6;
export const SPIDER_ENTERS = 28.9;
...
export function directFilm(seconds: number, portrait: boolean) {
  ...
  const times = index === 0
    ...
    : index === 3
      ? [0, .35, 1.3, 2, 2.7, 3.35, 3.6, 8, 13]
      : [0, .35, 1.5, 3, 4.3, 5.5, duration - 1.5, duration - .5, duration];
  const x = index === 3
    ? [-.75, -.75, .14, .37, .7, 1.45, 1.65, 1.65, 1.65]
  ...
  return {
    ...
    birds: { x: smoothPath(local, times, x), y: smoothPath(local, times, y), size: smoothPath(local, times, scale), bank: smoothPath(local, times, bank), visible: t > 1.4 && t < NEW_YORK_BIRDS_EXIT },
  };
}
...
export function deliveryAt(t: number, portrait: boolean) {
  ...
  return { x, y, angle, visible: t >= SPIDER_ENTERS + 6 && t < 40.1, anchorX: .69, anchorY: -.13 };
}
